...
export const ExportTab = memo(function ExportTab() {
  ...
  const configKey = `${modConfig.modId}|${modConfig.modName}|${modConfig.modAuthor}|${modConfig.modVersion}|${modConfig.modDescription}`;
  const audiosKey = JSON.stringify(audios.map(a => ({ id: a.id, category: a.category, files: a.files, looped: a.looped, jukebox: a.jukebox })));
  ...
  const headerClass = cn('border-2 rounded-xl p-4',
    tauri.isDesktop
  ...
  const headerTitleClass = cn('text-xl font-bold mb-2',
    tauri.isDesktop
    ...
    <div className="animate-fade-in space-y-4">
      <div className={headerClass}>
        ...
        <p className={cn('text-base', theme === 'dark' ? 'text-gray-300' : '')}>
          {tauri.isDesktop ? t('export.descNative') : t('export.descWeb')}
        </p>
        {audios.length > 0 && (
          <p className={cn('text-sm mt-2 font-mono opacity-80', theme === 'dark' ? 'text-gray-300' : 'text-gray-700')}>
            🎵 {audios.length} {t('export.audioCount')} | 📁 {reqFiles.length} {t('export.fileCount')}
          </p>
        )}
      </div>
        ...
        <div key={field} className={cn('rounded-lg overflow-hidden', theme === 'dark' ? 'border border-gray-700' : '')}>
          <div className={previewHeaderClass}>
            ...
            <button onClick={() => handleCopy(json, field)} className={cn('flex items-center gap-1.5 px-3 py-1 rounded text-sm text-white transition-colors', theme === 'dark' ? 'bg-blue-600 hover:bg-blue-500' : 'bg-[#4a90d9] hover:brightness-110')}>
              {copiedField === field ? t('export.copied') : t('export.copy')}
            </button>
          </div>
